...
void leer_odom(const geometry_msgs::TwistPtr mensaje)
{
	velocidad_actual = mensaje->linear.x;
}
...
int main(int argc, char **argv)
{
    ...
    wiringPiSetupGpio();
    pinMode(velocidad_izquierdo, PMW_OUTPUT);
    pinMode(Adelante_izquierdo, OUTPUT);
    pinMode(Atras_izquierdo, OUTPUT);

	ros::Subscriber imu_sub=n.subscribe("mimu", 1, &leer_imu);
