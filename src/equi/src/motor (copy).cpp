...
//float Kp = 20; //80 bien con vel-max 1
//float Ki = 0; // 20 sin velocida
//float Kd = 0.0; //0.2
//float Kpvel = 0; // bueno 0.0125
//float Kivel = 0;
//float Kdvel = 0;    // bueno 0.01

//float angulo_error = 0.0, angulo_error_anterior = 0.0, suma_error_angulo = 0.0;
//float angulo_equilibrio = -4.9, velocidad_actual = 0;
//float velocidad_enviada = 0.0, velocidad_error = 0.0, velocidad_error_anterior = 0.0, suma_velocidad_error = 0.0;
//---------------------------------------------------------------------
...
//	float PID_Kdvel;
    ...
    float angle_zero = -4.9;            // ¿-4.9?  x axle angle calibration
    float angular_velocity_zero = 0; //x axle angular velocity calibration
    ...
    while (ros::ok)
    {
     ...
     //ROS_WARN("Angulo Actual : %f,", angulo_actual);
              if (angulo_actual > 25.0 or angulo_actual < -35.0)
                 ...
                 else { balance_control_output = kp_balance * (angulo_actual - angle_zero) + kd_balance * (kalmanfilter.Gyro_y - angular_velocity_zero);
                        if (contador == 8)
                          {  contador = 0;
                             ...
                             speed_integeral += speed_filter; // ¿Velocidad?
                             speed_integeral += -velocidad_deseada;   // velocidad actual - velocidad deseada
